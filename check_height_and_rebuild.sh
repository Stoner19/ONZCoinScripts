	...
	do
		...
		then
			...
			then
				...
				bash onz_manager.bash stop
				sleep 5
				bash onz_manager.bash start
				sleep 2
			fi
		fi
...
find_newest_snap_rebuild(){
	...
	SNAPSHOTS=(
	  $HOME/ONZ-snapshot/snapshot/ ## this is assuming that you are running ONZ-snapshot (https://github.com/onz/ONZ-snapshot)
	  ## https://
	  ## https://
	  ## https://
	)
	...
	then
		date +"%Y-%m-%d %H:%M:%S || Newest snap: $BESTSNAP at block: $BESTSNAPBLOCK"
    		bash onz_manager.bash rebuild -u $BESTSNAP
	else
		date +"%Y-%m-%d %H:%M:%S || Newest snap: $BESTSNAP2 at block: $BESTSNAPBLOCK2"
    		bash onz_manager.bash rebuild -u $BESTSNAP2
	fi
}
...
local_height() {
	...
	then
		...
		ChangeDirectory ## Make sure we are in the correct directory
		bash onz_manager.bash reload  # 0.5.1 often solves short stucks by itself, but reload anyways :)
		sleep 20
		SyncState
